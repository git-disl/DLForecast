...
#training data
X = np.genfromtxt('embedding_0.csv', delimiter=',', dtype=float, autostrip=True)

# import csv
# with open('embedding0.csv',mode='r') as infile:
...
#         need=value[1:-1]
#
#         #need = np.fromstring(need, dtype=float, sep=' ')
#         #value=rows[1].strip()
#         #print (value.split())
#         v=[float(i) for i in need]
#         X={k:v for k,v in reader}
...
load_data=open('./delta_data_weight_scale10k/data0_weight.txt','r')
#load_data = open('./delta_data_weight_scale10k/delta_data0_weight.txt','r')
raw_data=load_data.read()
tmp = raw_data.splitlines()

x_train=np.zeros((15000,256))
...
for i,row in enumerate(tmp):
    ...
    #print (X[str(pair[0])].shape)
    # mmm=X[str(pair[0])].split()
    # left =[float(i) for i in mmm[1:-1]]
    # left=np.asarray(left)
    # print (left.shape)
    try:
        x_train[i] = np.concatenate((X[str(pair[0])], X[str(pair[1])]))
        if pair[2] > 10:
            y_train[i] = 0
        # elif pair[2] > 50:
        #     y_train[i] = 0.5
...
while i<15000:
    ...
    try:
        if a not in list2:
            i = i + 1
            x_train[i] = np.concatenate((X[str(a[0])], X[str(a[1])]))
            y_train[i] = 0
...
#test data
X1 = np.genfromtxt('embedding_1.csv', delimiter=',', dtype=float, autostrip=True)
...
load_data1 = open('./delta_data_weight_scale10k/delta_data1_weight.txt', 'r')
raw_data1 = load_data1.read()
tmp1 = raw_data1.splitlines()

x_test = np.zeros((15000, 256))
...
arg_test=np.reshape(arg_test,15000)
arg_predict=np.reshape(arg_predict,15000)
