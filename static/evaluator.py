...
class Evaluator(object):
    ...
    def _evaluate_ne_cmd(self, data_split, method_name, command, edge_embedding_methods, input_delim, output_delim,
        ...
        try:
            ...
            #X = np.genfromtxt('embedding_1.csv', delimiter=',', dtype=float,autostrip=True)

            print (X.shape)

            # import csv
            # W = csv.writer(open('embedding_9.csv', 'w'))
            # for row in X:
            #     W.writerow(row)
            ...
            import csv
            W=csv.writer(open('embedding00.csv','w'))
            for row in X.items():
                W.writerow(row)
    ...
    def evaluate_ne(self, data_split, X, method, edge_embed_method,
        ...
        #print (data_split.train_edges.shape)
