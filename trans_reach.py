...
#training data
X = np.genfromtxt('./embedding_scale10k_end/embedding_7.csv', delimiter=',', dtype=float, autostrip=True)
...
load_data = open('./delta_data_weight_scale10k_end/delta_data7_weight_end.txt','r')
raw_data=load_data.read()
tmp = raw_data.splitlines()

x_train=np.zeros((20000,256))
...
#test data
X1 = np.genfromtxt('./embedding_scale10k_end/embedding_8.csv', delimiter=',', dtype=float, autostrip=True)
...
load_data1 = open('./delta_data_weight_scale10k_end/delta_data8_weight_end.txt', 'r')
raw_data1 = load_data1.read()
tmp1 = raw_data1.splitlines()

x_test = np.zeros((20000, 256))
y_test = np.zeros((20000, 1))
...
arg_test=np.reshape(arg_test,20000)
arg_predict=np.reshape(arg_predict,20000)
